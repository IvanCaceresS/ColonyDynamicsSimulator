...
public partial class BaciloSystem : SystemBase
{
    protected override void OnUpdate()
    {
        ...
        Dependency=Entities.WithReadOnly(parentMap).ForEach((Entity entity,int entityInQueryIndex,ref LocalTransform transform,ref BaciloComponent organism)=>
        {   
            ...
            if(!organism.IsInitialCell&&organism.Parent!=Entity.Null&&parentMap.TryGetValue(organism.Parent,out ParentData pd))
            {
                if(transform.Scale<organism.SeparationThreshold*maxScale)
                {
                    ...
                    float off=math.lerp(0f,__LENGTH__+organism.SeparationThreshold,sp);
